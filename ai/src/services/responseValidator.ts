...
function validateLadderBlock(block: unknown, index: number): string[] {
  ...
  if (!isValidBlockType(block.type)) {
    errors.push(`Block ${index}: Invalid type "${block.type}". Must be one of: ${VALID_BLOCK_TYPES.join(', ')}`);
  }
...
function validateLadderStructure(ladder: unknown[]): { errors: string[]; warnings: string[] } {
  ...
  const coilCount = ladder.filter((block) => isRecord(block) && block.type === 'coil').length;
  if (coilCount === 0) {
...
export function validateAIResponse(response: unknown): { isValid: boolean; errors: string[]; warnings: string[] } {
  const errors: string[] = [];
  const warnings: string[] = [];

  if (!isRecord(response)) {
    return { isValid: false, errors: ['Response is not a valid object'], warnings };
  }
...
export { validateAIResponse as validateResponse };
