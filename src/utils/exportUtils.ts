 ...
 */
export function buildPLCopenXML(ladderData: LadderBlock[], instructionList: string): string {
    const now = new Date().toISOString().slice(0, 19);
    const rungs = splitIntoRungs(ladderData);
 ...
 */
export function buildGXWorksLabelCSV(ladderData: LadderBlock[]): string {
    const header = '"Label Name","Data Type","Class","Device","Comment"';
    const rows = ladderData.map((block) => {
